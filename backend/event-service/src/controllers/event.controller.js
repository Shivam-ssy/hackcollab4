...
exports.createEvent = async (req, res, next) => {
  try {
    ...
    const event = await Event.create({
      ...
      tags: tags || [],
      createdBy: userId,
      collegeId: req.user.collegeId || null, // Attach the user's collegeId to the event
      organizerName,
      image: image || ''
    });
    ...
    try {
      const notificationServiceUrl = 'https://hackcollab-notification-service.onrender.com' || 'http://localhost:8003';
...
exports.getAllEvents = async (req, res, next) => {
  try {
    ...
    // TENANT ISOLATION: 
    // If query has collegeId, use it (for public views of a college's events)
    if (req.query.collegeId) {
      query.collegeId = req.query.collegeId;
    } else if (req.user && req.user.role === 'COLLEGE_ADMIN') {
      // If logged in as COLLEGE_ADMIN and no specific query, scope to their college
      query.collegeId = req.user.collegeId;
    } else if (req.user && req.user.role === 'STUDENT') {
      // Students should only see their college events
      query.collegeId = req.user.collegeId;
    }
...
exports.updateEvent = async (req, res, next) => {
  try {
    ...
    const isSuperAdmin = req.user.role === 'SUPER_ADMIN' || (req.user.permissions && req.user.permissions.includes('MANAGE_PLATFORM'));
    const isCollegeAdminOfEvent = req.user.role === 'COLLEGE_ADMIN' && event.collegeId && event.collegeId.toString() === req.user.collegeId;
    
    if (event.createdBy.toString() !== req.user.id.toString() && !isSuperAdmin && !isCollegeAdminOfEvent) {
...
exports.getEventParticipants = async (req, res, next) => {
  try {
    ...
    const isAuthorized = 
      ...
      isSuperAdmin || isCollegeAdminOfEvent ||
