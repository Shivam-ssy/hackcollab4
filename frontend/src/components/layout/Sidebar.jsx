...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const isOrganizer = user && (user.role === 'organizer' || user.role === 'COLLEGE_ADMIN');
  const isSponsor = user && user.role === 'COMPANY_ADMIN';
  const isJudge = user && user.role === 'JUDGE';
  return (
    <>
      {/* Overlay */}
      {isOpen && (
      ...
      >
        ...
        <nav className="mt-4">
          ...
          <ul>
            ...
            {/* Sponsor Dashboard - Only visible to sponsors */}
            {isSponsor && (
              <li>
                <Link to="/sponsordashboard" className="flex items-center px-4 py-3 hover:bg-blue-700 rounded-md mx-2 transition-all duration-200">
                  <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Judge Dashboard - Only visible to judges */}
            {isJudge && (
              <li>
                <Link to="/judge-dashboard" className="flex items-center px-4 py-3 hover:bg-blue-700 rounded-md mx-2 transition-all duration-200">
                  <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {!isAdmin && !isOrganizer && !isSponsor && !isJudge && (
              <li>
